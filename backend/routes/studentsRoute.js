...
router.post("/signup", async (req, res) => {
    const { name, student_id, department, email, password, phoneNumber } =
        req.body;
    ...
    db.query(checkSql, [email, student_id], async (err, result) => {
        ...
        db.query(
            sql,
            [name, student_id, department, email, hashedPassword, phoneNumber],
            (err, result) => {
                if (err) {
                    console.error(err);
                    return res
                        ...
                        .json({
                            success: false,
                            message: "Error signing up the student",
                        });
                }
    ...
    try {
        ...
        if (!id || !name || !phone) {
            return res
                .status(400)
                .json({
                    message: "Please provide student ID, name, and phone.",
                });
        }

        const updateSql =
            "UPDATE student SET name = ?, sid = ?, phone = ? WHERE sid = ?";
...
router.post("/submit-course", auth, (req, res) => {
    ...
    db.query(studentCheckSql, [student_id], (studentErr, studentRows) => {
        if (studentErr) {
            ...
            return res
                .status(500)
                .json({ success: false, message: "Error checking student" });
        }
        ...
        if (studentRows.length > 0) {
            return res
                ...
                .json({
                    success: false,
                    message: "You can not take more courses",
                });
        }

        const insertSql =
            "INSERT INTO student_take_course (sid, ccode) VALUES (?, ?)";
        db.query(
            insertSql,
            [student_id, selectedCourse],
...
router.delete("/drop-course", auth, (req, res) => {
    ...
    const enrollmentCheckSql =
        "SELECT * FROM student_take_course WHERE sid = ? AND ccode = ?";
    db.query(
        enrollmentCheckSql,
        [student_id, selectedCourse],
        (enrollmentErr, enrollmentRows) => {
            if (enrollmentErr) {
            ...
            const deleteSql =
                "DELETE FROM student_take_course WHERE sid = ? AND ccode = ?";
            db.query(
                deleteSql,
                [student_id, selectedCourse],
